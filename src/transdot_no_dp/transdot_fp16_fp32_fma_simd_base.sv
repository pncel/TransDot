...
module transdot_fp16_fp32_fma_simd #(
  ...
  // FP8 parameters
  localparam int unsigned SUPER_EXP_BITS_FP8 = 4;
  ...
  localparam int unsigned EXP_WIDTH_FP8 = fpnew_pkg::maximum(SUPER_EXP_BITS_FP8 + 2, LZC_RESULT_WIDTH_FP8); // 6
  localparam int unsigned SHIFT_AMOUNT_WIDTH_FP8 = $clog2(3 * PRECISION_BITS_FP8 + 5); // 5
  ...
  for (genvar fmt = 0; fmt < int'(NUM_FORMATS); fmt++) begin : fmt_init_inputs_simd
    ...
    if (fmt==2 || fmt==3) begin : active_format // FP16 and FP8
      localparam fpnew_pkg::fp_format_e FpFormat = fpnew_pkg::fp_format_e'(fmt);
      logic [2:0][FP_WIDTH-1:0] trimmed_ops;
  ...
  // Start --- FP8 SIMD lanes (fp8_1: byte[15:8], fp8_2: byte[31:24]) ---
  logic src_is_fp8;
  assign src_is_fp8 = (src_fmt_q == fpnew_pkg::FP8);

  logic        [2:0][WIDTH/4-1:0] operands_q_fp8_1;  // byte 1
  logic        [2:0][WIDTH/4-1:0] operands_q_fp8_2;  // byte 3
  for (genvar op = 0; op < 3; op++) begin : gen_fp8_operand_extract
    assign operands_q_fp8_1[op]    = operands_q[op][15:8];
    assign operands_q_fp8_2[op]    = operands_q[op][31:24];
  end

  // --- FP8 lane 1 (byte[15:8]) classifier ---
  logic        [NUM_FORMATS-1:0][2:0]                        fmt_sign_fp8_1;
  logic signed [NUM_FORMATS-1:0][2:0][SUPER_EXP_BITS_FP8-1:0] fmt_exponent_fp8_1;
  logic        [NUM_FORMATS-1:0][2:0][SUPER_MAN_BITS_FP8-1:0] fmt_mantissa_fp8_1;
  fpnew_pkg::fp_info_t [NUM_FORMATS-1:0][2:0] info_q_fp8_1;

  for (genvar fmt = 0; fmt < int'(NUM_FORMATS); fmt++) begin : fmt_init_inputs_fp8_1
    localparam int unsigned FP_WIDTH = fpnew_pkg::fp_width(fpnew_pkg::fp_format_e'(fmt));
    localparam int unsigned EXP_BITS = fpnew_pkg::exp_bits(fpnew_pkg::fp_format_e'(fmt));
    localparam int unsigned MAN_BITS = fpnew_pkg::man_bits(fpnew_pkg::fp_format_e'(fmt));

    if (fmt==3) begin : active_format // FP8 only
      localparam fpnew_pkg::fp_format_e FpFormat = fpnew_pkg::fp_format_e'(fmt);
      logic [2:0][FP_WIDTH-1:0] trimmed_ops;
      ...
      for (genvar op = 0; op < 3; op++) begin : gen_operands
        ...
        assign fmt_mantissa_fp8_1[fmt][op]  = {info_q_fp8_1[fmt][op].is_normal, operands_q_fp8_1[op][MAN_BITS-1:0]} <<
                                               (SUPER_MAN_BITS_FP8 - MAN_BITS);
      end
    end else begin : inactive_format
      assign info_q_fp8_1[fmt]         = '{default: fpnew_pkg::DONT_CARE};
      assign fmt_sign_fp8_1[fmt]       = fpnew_pkg::DONT_CARE;
      assign fmt_exponent_fp8_1[fmt]   = '{default: fpnew_pkg::DONT_CARE};
      assign fmt_mantissa_fp8_1[fmt]   = '{default: fpnew_pkg::DONT_CARE};
    end
  end

  // --- FP8 lane 2 (byte[31:24]) classifier ---
  logic        [NUM_FORMATS-1:0][2:0]                        fmt_sign_fp8_2;
  logic signed [NUM_FORMATS-1:0][2:0][SUPER_EXP_BITS_FP8-1:0] fmt_exponent_fp8_2;
  logic        [NUM_FORMATS-1:0][2:0][SUPER_MAN_BITS_FP8-1:0] fmt_mantissa_fp8_2;
  ...
  for (genvar fmt = 0; fmt < int'(NUM_FORMATS); fmt++) begin : fmt_init_inputs_fp8_2
    ...
    if (fmt==3) begin : active_format // FP8 only
      ...
      assign info_q_fp8_2[fmt]         = '{default: fpnew_pkg::DONT_CARE};
      assign fmt_sign_fp8_2[fmt]       = fpnew_pkg::DONT_CARE;
      assign fmt_exponent_fp8_2[fmt]   = '{default: fpnew_pkg::DONT_CARE};
      assign fmt_mantissa_fp8_2[fmt]   = '{default: fpnew_pkg::DONT_CARE};
    end
  end
  // End --- FP8 lanes classifier ---
  ...
  // Start --- FP8 SIMD lanes op_select ---
  ...
  // Start --- FP8 SIMD lanes input_classify ---
  logic any_operand_inf_fp8_1, any_operand_nan_fp8_1, signalling_nan_fp8_1;
  logic effective_subtraction_fp8_1, tentative_sign_fp8_1;
  ...
  logic any_operand_inf_fp8_2, any_operand_nan_fp8_2, signalling_nan_fp8_2;
  logic effective_subtraction_fp8_2, tentative_sign_fp8_2;
  ...
  // End --- FP8 SIMD lanes input_classify ---
  ...
  // Start --- FP8 SIMD lanes special_results ---
  ...
  // End --- FP8 SIMD lanes special_results ---
  ...
  logic [SHIFT_AMOUNT_WIDTH_SIMD-1:0] addend_shamt_simd;
  ...
  fpnew_exponent_datapath #(
    ...
    .exponent_a_i(operand_a_simd.exponent),
    .exponent_b_i(operand_b_simd.exponent),
    .exponent_c_i(operand_c_simd.exponent),
    .mantissa_c_i(operand_c_simd.mantissa),
    .info_a_i(info_a_simd),
    .info_b_i(info_b_simd),
    .info_c_i(info_c_simd),
    ...
    .exponent_addend_o(exponent_addend_simd),
    .exponent_product_o(exponent_product_simd),
    .exponent_difference_o(exponent_difference_simd),
    .tentative_exponent_o(tentative_exponent_simd),
    .addend_shamt_o(addend_shamt_simd),
    .addend_normalize_shamt_o(addend_normalize_shamt_simd)
  );
  // End --- SIMD lane 1 datapath ---

  // Start --- FP8 SIMD lanes exponent_datapath ---
  ...
  fpnew_exponent_datapath #(
    ...
    .exponent_a_i(operand_a_fp8_1.exponent),
    .exponent_b_i(operand_b_fp8_1.exponent),
    .exponent_c_i(operand_c_fp8_1.exponent),
    .mantissa_c_i(operand_c_fp8_1.mantissa),
    .info_a_i(info_a_fp8_1),
    .info_b_i(info_b_fp8_1),
    .info_c_i(info_c_fp8_1),
    .src_fmt_i(src_fmt_q),
    .src2_fmt_i(src2_fmt_q),
    .dst_fmt_i(dst_fmt_q),
    .exponent_addend_o(exponent_addend_fp8_1),
    .exponent_product_o(exponent_product_fp8_1),
    .exponent_difference_o(exponent_difference_fp8_1),
    .tentative_exponent_o(tentative_exponent_fp8_1),
    .addend_shamt_o(addend_shamt_fp8_1),
    .addend_normalize_shamt_o(addend_normalize_shamt_fp8_1)
  );
  ...
  fpnew_exponent_datapath #(
    ...
    .exponent_a_i(operand_a_fp8_2.exponent),
    .exponent_b_i(operand_b_fp8_2.exponent),
    .exponent_c_i(operand_c_fp8_2.exponent),
    .mantissa_c_i(operand_c_fp8_2.mantissa),
    .info_a_i(info_a_fp8_2),
    .info_b_i(info_b_fp8_2),
    .info_c_i(info_c_fp8_2),
    .src_fmt_i(src_fmt_q),
    .src2_fmt_i(src2_fmt_q),
    .dst_fmt_i(dst_fmt_q),
    .exponent_addend_o(exponent_addend_fp8_2),
    .exponent_product_o(exponent_product_fp8_2),
    .exponent_difference_o(exponent_difference_fp8_2),
    .tentative_exponent_o(tentative_exponent_fp8_2),
    .addend_shamt_o(addend_shamt_fp8_2),
    .addend_normalize_shamt_o(addend_normalize_shamt_fp8_2)
  );
  // End --- FP8 SIMD lanes exponent_datapath ---
  ...
  multiplier_unsigned #(
    .PRECISION_BITS ( PRECISION_BITS_SIMD )
  ) i_mantissa_multiply_simd (
    .a_i     ( mantissa_a_simd ),
    .b_i     ( mantissa_b_simd ),
    .product_o ( product_simd   )
  );
  assign product_shifted_simd = product_simd << 2; // constant shift
  // End --- SIMD lane 1 datapath ---

  // Start --- FP8 SIMD lanes multiplier ---
  logic [PRECISION_BITS_FP8-1:0]  mantissa_a_fp8_1, mantissa_b_fp8_1, mantissa_c_fp8_1;
  logic [2*PRECISION_BITS_FP8-1:0]  product_fp8_1;
  logic [3*PRECISION_BITS_FP8+3:0]  product_shifted_fp8_1;
  ...
  multiplier_unsigned #(.PRECISION_BITS(PRECISION_BITS_FP8)) i_mantissa_multiply_fp8_1 (
    .a_i(mantissa_a_fp8_1), .b_i(mantissa_b_fp8_1), .product_o(product_fp8_1));
  assign product_shifted_fp8_1 = product_fp8_1 << 2;

  logic [PRECISION_BITS_FP8-1:0]  mantissa_a_fp8_2, mantissa_b_fp8_2, mantissa_c_fp8_2;
  logic [2*PRECISION_BITS_FP8-1:0]  product_fp8_2;
  logic [3*PRECISION_BITS_FP8+3:0]  product_shifted_fp8_2;
  ...
  multiplier_unsigned #(.PRECISION_BITS(PRECISION_BITS_FP8)) i_mantissa_multiply_fp8_2 (
    .a_i(mantissa_a_fp8_2), .b_i(mantissa_b_fp8_2), .product_o(product_fp8_2));
  assign product_shifted_fp8_2 = product_fp8_2 << 2;
  // End --- FP8 SIMD lanes multiplier ---
  ...
  fpnew_addend_datapath #(
    ...
    .mantissa_c_i            (mantissa_c_simd),
    .product_shifted_i       (product_shifted_simd),
    .addend_shamt_i          (addend_shamt_simd),
    .effective_subtraction_i (effective_subtraction_simd),
    .tentative_sign_i        (tentative_sign_simd),
    .sticky_before_add_o     (sticky_before_add_simd),
    .sum_o                   (sum_simd),
    .final_sign_o            (final_sign_simd)
  );
  // End --- SIMD lane 1 datapath ---

  // Start --- FP8 SIMD lanes addend_datapath ---
  ...
  fpnew_addend_datapath #(
    ...
    .mantissa_c_i            (mantissa_c_fp8_1),
    .product_shifted_i       (product_shifted_fp8_1),
    .addend_shamt_i          (addend_shamt_fp8_1),
    .effective_subtraction_i (effective_subtraction_fp8_1),
    .tentative_sign_i        (tentative_sign_fp8_1),
    .sticky_before_add_o     (sticky_before_add_fp8_1),
    .sum_o                   (sum_fp8_1),
    .final_sign_o            (final_sign_fp8_1)
  );
  ...
  fpnew_addend_datapath #(
    ...
    .mantissa_c_i            (mantissa_c_fp8_2),
    .product_shifted_i       (product_shifted_fp8_2),
    .addend_shamt_i          (addend_shamt_fp8_2),
    .effective_subtraction_i (effective_subtraction_fp8_2),
    .tentative_sign_i        (tentative_sign_fp8_2),
    .sticky_before_add_o     (sticky_before_add_fp8_2),
    .sum_o                   (sum_fp8_2),
    .final_sign_o            (final_sign_fp8_2)
  );
  // End --- FP8 SIMD lanes addend_datapath ---
  ...
  logic                  [0:NUM_MID_REGS]                         mid_pipe_is_fp8_q;
  ...
  assign mid_pipe_is_fp8_q[0]      = src_is_fp8;
  ...
  for (genvar i = 0; i < NUM_MID_REGS; i++) begin : gen_inside_pipeline
    ...
    `FFL(mid_pipe_is_fp8_q[i+1],     mid_pipe_is_fp8_q[i],     reg_ena, '0)
  ...
  assign is_fp8_q                = mid_pipe_is_fp8_q[NUM_MID_REGS];
  ...
  // End --- FP8 SIMD lanes mid pipeline ---
  ...
  // Start --- FP8 SIMD lanes normalization ---
  ...
  // End --- FP8 SIMD lanes normalization ---
  ...
  fpnew_round_classify_stage #(
    ...
    .SUPER_MAN_BITS(SUPER_MAN_BITS_SIMD),
    .NUM_FORMATS(NUM_FORMATS),
    .FpFmtConfig(6'b001100)  // SIMD lane: FP16 + FP8 (format indices 2 and 3, big-endian [0:5])
  ) i_round_classify_stage_simd (
    .final_exponent_i       (final_exponent_simd),
    .final_mantissa_i       (final_mantissa_simd),
    .final_sign_i           (final_sign_q_simd),
    .sticky_after_norm_i    (sticky_after_norm_simd),
    .dst_fmt_i              (dst_fmt_q2),
    .rnd_mode_i             (rnd_mode_q),
    .effective_subtraction_i(effective_subtraction_q_simd),
    .sum_sticky_bits_i      (sum_sticky_bits_simd),
  ...
  // Start --- FP8 SIMD lanes round_classify ---
  logic [1:0]                               round_sticky_bits_fp8_1;
  logic of_before_round_fp8_1, of_after_round_fp8_1;
  logic uf_before_round_fp8_1, uf_after_round_fp8_1;
  ...
  fpnew_round_classify_stage #(
    ...
    .SUPER_MAN_BITS(SUPER_MAN_BITS_FP8),
    .NUM_FORMATS(NUM_FORMATS),
    .FpFmtConfig(6'b000100)  // FP8 only (format index 3, big-endian [0:5])
  ) i_round_classify_stage_fp8_1 (
    .final_exponent_i       (final_exponent_fp8_1),
    .final_mantissa_i       (final_mantissa_fp8_1),
    .final_sign_i           (final_sign_q_fp8_1),
    .sticky_after_norm_i    (sticky_after_norm_fp8_1),
    .dst_fmt_i              (dst_fmt_q2),
    .rnd_mode_i             (rnd_mode_q),
    .effective_subtraction_i(effective_subtraction_q_fp8_1),
    .sum_sticky_bits_i      (sum_sticky_bits_fp8_1),
  ...
  logic of_before_round_fp8_2, of_after_round_fp8_2;
  logic uf_before_round_fp8_2, uf_after_round_fp8_2;
  ...
  fpnew_round_classify_stage #(
    ...
    .SUPER_MAN_BITS(SUPER_MAN_BITS_FP8),
    .NUM_FORMATS(NUM_FORMATS),
    .FpFmtConfig(6'b000100)  // FP8 only
  ) i_round_classify_stage_fp8_2 (
    .final_exponent_i       (final_exponent_fp8_2),
    .final_mantissa_i       (final_mantissa_fp8_2),
    .final_sign_i           (final_sign_q_fp8_2),
    .sticky_after_norm_i    (sticky_after_norm_fp8_2),
    .dst_fmt_i              (dst_fmt_q2),
    .rnd_mode_i             (rnd_mode_q),
    .effective_subtraction_i(effective_subtraction_q_fp8_2),
    .sum_sticky_bits_i      (sum_sticky_bits_fp8_2),
  ...
  // End --- FP8 SIMD lanes round_classify ---
  ...
  assign result_d_normal = result_is_special_q ? special_result_q : regular_result;
  assign status_d_normal = result_is_special_q ? special_status_q : regular_status;

  // Start --- SIMD lane 1 datapath ---
  logic [WIDTH/2-1:0]     regular_result_simd;
  fpnew_pkg::status_t   regular_status_simd;
  // Assemble regular result
  assign regular_result_simd = fmt_result_simd[dst_fmt_q2];
  ...
  // Select output depending on special case detection
  assign result_d_simd = result_is_special_q_simd ? special_result_q_simd[WIDTH/2-1:0] : regular_result_simd;
  assign status_d_simd = result_is_special_q_simd ? special_status_q_simd : regular_status_simd;
  // End --- SIMD lane 1 datapath ---

  // Start --- FP8 SIMD lanes result selection ---
  // --- fp8_1 ---
  logic [WIDTH/4-1:0]     regular_result_fp8_1;
  fpnew_pkg::status_t   regular_status_fp8_1;
  assign regular_result_fp8_1 = fmt_result_fp8_1[dst_fmt_q2];
  assign regular_status_fp8_1.NV = 1'b0;
  assign regular_status_fp8_1.DZ = 1'b0;
  assign regular_status_fp8_1.OF = of_before_round_fp8_1 | of_after_round_fp8_1;
  assign regular_status_fp8_1.UF = uf_after_round_fp8_1 & regular_status_fp8_1.NX;
  ...
  assign result_d_fp8_1 = result_is_special_q_fp8_1 ? special_result_q_fp8_1[WIDTH/4-1:0] : regular_result_fp8_1;
  assign status_d_fp8_1 = result_is_special_q_fp8_1 ? special_status_q_fp8_1 : regular_status_fp8_1;

  // --- fp8_2 ---
  logic [WIDTH/4-1:0]     regular_result_fp8_2;
  fpnew_pkg::status_t   regular_status_fp8_2;
  assign regular_result_fp8_2 = fmt_result_fp8_2[dst_fmt_q2];
  assign regular_status_fp8_2.NV = 1'b0;
  assign regular_status_fp8_2.DZ = 1'b0;
  assign regular_status_fp8_2.OF = of_before_round_fp8_2 | of_after_round_fp8_2;
  assign regular_status_fp8_2.UF = uf_after_round_fp8_2 & regular_status_fp8_2.NX;
  ...
  assign result_d_fp8_2 = result_is_special_q_fp8_2 ? special_result_q_fp8_2[WIDTH/4-1:0] : regular_result_fp8_2;
  assign status_d_fp8_2 = result_is_special_q_fp8_2 ? special_status_q_fp8_2 : regular_status_fp8_2;
  // End --- FP8 SIMD lanes result selection ---

  logic [WIDTH-1:0]   result_d;
  fpnew_pkg::status_t status_d;

  // Final result mux: scalar / FP16 SIMD / FP8 SIMD
  logic fp8_simd_mode;
  assign fp8_simd_mode = simd_enable_q & is_fp8_q;

  always_comb begin
    if (fp8_simd_mode) begin
      // FP8 SIMD: 4 bytes = {fp8_2[31:24], simd[7:0](byte2), fp8_1[15:8], main[7:0]}
      result_d = {result_d_fp8_2, result_d_simd[7:0], result_d_fp8_1, result_d_normal[0 +: WIDTH/4]};
      status_d = status_d_fp8_2 | status_d_simd | status_d_fp8_1 | status_d_normal;
    end else if (simd_enable_q) begin
      // FP16 SIMD: 2 halves = {simd[31:16], main[15:0]}
      result_d = {result_d_simd, result_d_normal[0 +: WIDTH/2]};
      status_d = status_d_simd | status_d_normal;
    end else begin
      // Scalar mode
      result_d = result_d_normal;
      status_d = status_d_normal;
    end
  end
...
module fpnew_op_select #(
  ...
  always_comb begin : op_select
    ...
    unique case (op_i)
      fpnew_pkg::FMADD,
      fpnew_pkg::TDOT_SIMD_FMADD: ; // do nothing (SIMD mode handled by simd_enable_i)
